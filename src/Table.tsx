...
abstract class AbstractSortableColumn implements ISortableColumn {
    ...
    public getColumn(getCellData: ICellLookup, sortColumn: ISortCallback) {
        const cellRenderer = (rowIndex: number, columnIndex: number) => {
            // console.log("cellRenderer", rowIndex, columnIndex);
...
export class Table extends React.PureComponent {
    public state = {
        ...
        data: Array(10).fill(null).flatMap(() => sumo),
    ...
    private handleVisibleCellsChange = (rowIndices: RowIndices, columnIndices: ColumnIndices) => {
        console.log("rowIndices", rowIndices);
        console.log("columnIndices", columnIndices);
        console.log("sortedIndexMap", this.state.sortedIndexMap);
        this.setState({ visibleRows: rowIndices, visibleColumns: columnIndices });
    };
            ...
            <HotkeysProvider>
                <div style={{ height: '100vh' }}>
                    <Table2
                        ...
                        numRows={Math.max(5000000, numRows)}
    ...
    private getCellData = (rowIndex: number, columnIndex: number) => {
        ...
        if (rowIndex < 0 || rowIndex >= this.state.data.length || columnIndex < 0 || columnIndex >= this.state.columns.length) {
            return 'bla';
        }
